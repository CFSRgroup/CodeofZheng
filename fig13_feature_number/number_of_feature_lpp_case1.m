...
for i=1:K
    ...
    rand('state',0)
    nb=NaiveBayes.fit(xtr_1(:,1:j),ytr_1); 
    yte_p=predict(nb,xte_1(:,1:j));
...
for i=1:K
    ...
    rand('state',0)
    nb=NaiveBayes.fit(xtr_2(:,1:j),ytr_2); 
    yte_p=predict(nb,xte_2(:,1:j));
...
y_3=[0*ones(length(y_medium),1);1*ones(length(y_high),1)];
    rand('state',0)
  model_1=glmfit(x_1(:,1:j),[y_1 ones(12600,1)],'binomial','link','logit');
  model_2=glmfit(x_2(:,1:j),[y_2 ones(12600,1)],'binomial','link','logit');
  model_3=glmfit(x_3(:,1:j),[y_3 ones(12600,1)],'binomial','link','logit');
  yte_p_1=round(glmval(model_1,xte_1(:,1:j),'logit'));
  yte_p_2=round(glmval(model_2,xte_1(:,1:j),'logit'));
  yte_p_3=round(glmval(model_3,xte_1(:,1:j),'logit'));
   for z=1:2700
...
y_3=[0*ones(length(y_medium),1);1*ones(length(y_high),1)];
  ...
  model_2=glmfit(x_2(:,1:j),[y_2 ones(12600,1)],'binomial','link','logit');
  model_3=glmfit(x_3(:,1:j),[y_3 ones(12600,1)],'binomial','link','logit');
  yte_p_1=round(glmval(model_1,xte_2(:,1:j),'logit'));
  yte_p_2=round(glmval(model_2,xte_2(:,1:j),'logit'));
  yte_p_3=round(glmval(model_3,xte_2(:,1:j),'logit'));
   for z=1:2700
    yte_p(z,:)=ovo_code(yte_p_1(1+(z-1)*1,:),yte_p_2(1+(z-1)*1,:),yte_p_3(1+(z-1)*1,:));
   end
    %目标类别确定之后的混淆矩阵
...
for i=1:K
    ...
    yte_1=cell2mat(yte_all_1(i,:));
    rand('state',0)
    mdl=fitcknn(xtr_1(:,1:j),ytr_1,'NumNeighbors',39);
    yte_p=predict(mdl,xte_1(:,1:j));
    cte(:,:,i,j)=cfmatrix(yte_1,yte_p);%计算混淆矩阵
    [acc_low(i,j),acc_medium(i,j),acc_high(i,j),Acc(i,j),sen(i,j),spe(i,j),pre(i,j),npv(i,j),f1(i,j),fnr(i,j),fpr(i,j),fdr(i,j),foe(i,j),mcc(i,j),bm(i,j),mk(i,j),ka(i,j)] = per_thrtotwo(cte(:,:,i,j)); %混淆矩阵指标
    TestingAccuracy(i,j)=Acc(i,j);
...
for i=1:K
    ...
    yte_2=cell2mat(yte_all_2(i,:));
    rand('state',0)
    mdl=fitcknn(xtr_2(:,1:j),ytr_2,'NumNeighbors',39);
    yte_p=predict(mdl,xte_2(:,1:j));
    cte(:,:,i,j)=cfmatrix(yte_2,yte_p);%计算混淆矩阵
    [acc_low(i,j),acc_medium(i,j),acc_high(i,j),Acc(i,j),sen(i,j),spe(i,j),pre(i,j),npv(i,j),f1(i,j),fnr(i,j),fpr(i,j),fdr(i,j),foe(i,j),mcc(i,j),bm(i,j),mk(i,j),ka(i,j)] = per_thrtotwo(cte(:,:,i,j)); %混淆矩阵指标
    TestingAccuracy(i,j)=Acc(i,j);
...
for i=1:K
    ...
    rand('state',0)
    net=patternnet(120);
    net=train(net,xtr_1(:,1:j)',ind2vec((ytr_1)')); 
    yte_p=net(xte_1(:,1:j)');
    yte_p=vec2ind(yte_p);
    yte_p=yte_p';
    cte(:,:,i,j)=cfmatrix(yte_1,yte_p);%计算混淆矩阵
...
for i=1:K
    ...
    rand('state',0)
    net=patternnet(120);
    net=train(net,xtr_2(:,1:j)',ind2vec((ytr_2)'));
    yte_p=net(xte_2(:,1:j)');
    yte_p=vec2ind(yte_p);
    yte_p=yte_p';
    cte(:,:,i,j)=cfmatrix(yte_2,yte_p);%计算混淆矩阵
...
for i=1:K
    ...
    yte_1=cell2mat(yte_all_1(i,:));
    rand('state',0)
    [TrainingTime(i,j),TrainingAccuracy(i,j),elm_model,label_index_expected] = elm_train_m(xtr_1(:,1:j),ytr_1, 1, 390, 'sig', 2^0);%ELM只认1，2类标签，不认0，1   elm_train_m()最后一个参数就是激活函数参数
...
save F:\matlab\trial_procedure\study_1\fig13_feature_number\subject8_case1_lpp acc_nb_case1 acc_lr_case1 acc_knn_case1 acc_ann_case1 acc_elm_case1
