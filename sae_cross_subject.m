...
% end
% 
% % m11=cte(:,:,1,1); m12=cte(:,:,2,1); m13=cte(:,:,3,1); m14=cte(:,:,4,1); m15=cte(:,:,5,1); m16=cte(:,:,6,1); m17=cte(:,:,7,1); m18=cte(:,:,8,1);
% % save F:\matlab\trial_procedure\study_1\matrix_square_figure\SAE\case1_session1 m11 m12 m13 m14 m15 m16 m17 m18
% 
% % subplot(2,1,1);
...
% % TrainingAccuracy(i,j)=length(find(predict_train==ytr_2))/size(ytr_2,1);
%     
% cte_1(:,:,i,j)=cfmatrix(yte_2,yte_p);%计算混淆矩阵
% 
% [acc_low(i,j),acc_medium(i,j),acc_high(i,j),Acc(i,j),sen(i,j),spe(i,j),pre(i,j),npv(i,j),f1(i,j),fnr(i,j),fpr(i,j),fdr(i,j),foe(i,j),mcc(i,j),bm(i,j),mk(i,j),ka(i,j)] = per_thrtotwo(cte(:,:,i,j)); %混淆矩阵指标
% mean_acc(i,j)=(acc_low(i,j)+acc_medium(i,j)+acc_high(i,j))/3;
% 
...
% m21=cte_1(:,:,1,1); m22=cte_1(:,:,2,1); m23=cte_1(:,:,3,1); m24=cte_1(:,:,4,1); m25=cte_1(:,:,5,1); m26=cte_1(:,:,6,1); m27=cte_1(:,:,7,1); m28=cte_1(:,:,8,1);
% save F:\matlab\trial_procedure\study_1\matrix_square_figure\SAE\case1_session2 m21 m22 m23 m24 m25 m26 m27 m28
...
% m31=cte_1(:,:,1,1); m32=cte_1(:,:,2,1); m33=cte_1(:,:,3,1); m34=cte_1(:,:,4,1); m35=cte_1(:,:,5,1); m36=cte_1(:,:,6,1);
% save F:\matlab\trial_procedure\study_1\matrix_square_figure\SAE\case2_session1 m31 m32 m33 m34 m35 m36
...
% 
% m41=cte(:,:,1,1); m42=cte(:,:,2,1); m43=cte(:,:,3,1); m44=cte(:,:,4,1); m45=cte(:,:,5,1); m46=cte(:,:,6,1);
% save F:\matlab\trial_procedure\study_1\matrix_square_figure\SAE\case2_session2 m41 m42 m43 m44 m45 m46